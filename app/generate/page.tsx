    ...
    try {
      ...
      const authoredDecision = await routeEditorialInteriorsV1Export({
        company: {
          ...
          companyType: typeof optimizedCompanyData.companyType === "string" ? optimizedCompanyData.companyType : profile.companyType,
          industry: typeof optimizedCompanyData.industry === "string" ? optimizedCompanyData.industry : "",
          customerType: typeof optimizedCompanyData.customerType === "string" ? optimizedCompanyData.customerType : "",
      ...
      if (process.env.NODE_ENV !== "production") {
        ...
        console.debug("[authored-export-accounting]", JSON.stringify({
          ...
          images: [
            ...
            ...projectOptimization.images.map((image, index) => ({ purpose: "project_image", provenance: "user_upload", originalFormat: /^data:image\/png/i.test(authoredProjects[index]?.imageUrl ?? "") ? "PNG" : "JPEG", originalWidth: image.originalWidth, originalHeight: image.originalHeight, originalBytes: image.originalBytes, optimizedFormat: image.format, optimizedWidth: image.width, optimizedHeight: image.height, optimizedBytes: image.optimizedBytes, pdfUsages: 1, deduplicated: true, passedAuthoredOptimization: true })),
          ],
