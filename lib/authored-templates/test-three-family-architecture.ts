...
const run = async () => {
  ...
  for (const remaining of [1, 2, 3] as const) {
    ...
    assert(plan.compatible, `${remaining}-item use-case continuation must plan.`); if (!plan.compatible) continue; const terminal = plan.plan.pages.at(-2)!; assert(terminal.templateId === `product-tech-v1.use-cases-continuation-${remaining}` && plan.plan.pages.at(-1)?.templateId === "product-tech-v1.closing", `${remaining}-item use-case continuation must select its exact fixed template before closing.`); assert(validateDocumentCoverage(units, plan.plan).complete, `${remaining}-item use-case continuation must preserve complete exact-once coverage.`);
  }
