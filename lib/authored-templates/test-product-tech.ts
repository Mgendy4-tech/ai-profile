...
const run = async () => {
  ...
  for (const remaining of [1, 2, 3, 4] as const) {
    ...
    const terminal = planned.result.plan.pages.at(-2)!; assert(terminal.templateId === `product-tech-v1.features-continuation-${remaining}` && planned.result.plan.pages.at(-1)?.templateId === "product-tech-v1.closing", `Exactly ${remaining} remaining features must select the fixed continuation before closing.`);
